...
├── templates/{experiment,protocol,sample,project}.md, experiment.{Type}.md overrides
...
| `export` | one Markdown bundle of selected experiments plus what they reference, with a primer for the model; `--format eln` writes an RO-Crate `.eln` archive instead |
| `complete` | close-out: refuses if Results/Interpretation are empty, sets `status`/`date_completed`, writes `{ID}_snapshot_{date}.html` and `{ID}_MANIFEST.sha256`, re-indexes |
| `verify` | compares a completed experiment's files with its manifest (missing / modified / added) |
| `render` | one note as a self-contained HTML file, images embedded |
...
- `scripts/eln.py` with all commands above; 57 unit tests
...
- The top-level record ships empty, ready to drop into a synced folder;
  the worked examples live in `sandbox/`, one of them closed out with a
  manifest and snapshot
- `.eln` export validated against the reference RO-Crate library; fifteen
  vendored K-Dense skills with a per-skill requirements table
