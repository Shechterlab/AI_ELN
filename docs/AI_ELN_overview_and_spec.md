...
4. **Low friction for people without computer skills.** Double-click
   launchers ask plain questions; the tool names everything; the note opens
   in whatever editor the person has.
...
├── scripts/eln.py, scripts/sync_skills.py
├── launchers/                                 double-click entry points (Mac + Windows)
...
- `scripts/eln.py` with all commands above; 48 unit tests
- Double-click launchers for Mac and Windows with first-run setup
...
- A web/GUI front end (the launchers cover the creation path; the viewing
  path is any Markdown editor)
